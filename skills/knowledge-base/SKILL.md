...
1. **Recall (deterministic):** `grep` `KNOWLEDGE-INDEX.md` for the current problem's keywords, or filter by `Category`.
...
7. **Health check — is this a graveyard? (cheap; do it here, never on a calendar.)** Count the `read` rows in `~/.claude/knowledge/CONSULT-LOG.tsv` since its last `write` row: that's how many times the KB was actually consulted since the previous lesson was recorded.
   - **This is a smoke detector, not a dashboard.** The only strongly actionable signal is **≈0 reads across several records** — meaning the CONSULT triggers aren't firing, the categories don't match the work being done, or the `CLAUDE.md` wiring is broken. All three are diagnosable; say so rather than silently continuing. Any healthy number means it's alive — stop looking. **Do not build analytics on this** (that's KB-0002's mistake in a new costume: machinery where reading it yourself is better).
   - **The log is written by a `PostToolUse` hook — the harness writes it, not you.** That's the entire point: a log *you* maintained would share the exact failure it measures, since an agent that forgets to consult also forgets to log. Never hand-write entries into it; if the hook is broken, fix the hook.
   - **It needs no watermark.** Each RECORD appends its own `write` row, which becomes the next boundary automatically — so KB-0009 is satisfied structurally here rather than by bookkeeping.
   - An empty log is only meaningful *after* you've recorded something with it installed. On the first record after install, there's nothing to compare against — note that and move on.
